...
def updated_priors_classification(A, seeds, labels, pis, max_dim, method = "ranks", add_smoothing = 1, test = False, alpha = 0.1, params = "", model = "", X_hat = "", acorn = 1234):
    from math import log
    ...
    if params == "" and method == "ranks":
        ...
        params = estimate_means(A, seeds, labels, return_argsort = True)[1]

    if test:
        weights = get_weights(A, seeds, labels)
    ...
    if method == "ranks":
        if test:
            if add_smoothing > 0: # If we want to transform our p-value (instead of just accept/reject)
                p_transformed = np.exp(np.log(K*1000)*p_meta)
                #print(p_transformed, '\n')
        ...
        sims = weight_likelihoods(A, seeds, labels, params = params, model = model, method = method)

    elif method == 'KS':
        seed_weights = get_weights(A, seeds, labels)
        weights = [get_weights(A, seeds, labels, int(unlabeled[i])) for i in range(len(unlabeled))]
        sims = [get_p_multiple_KS(weights[i], seed_weights) for i in range(len(unlabeled))]
    ...
    A_stripped = strip_weights(A)
    if type(X_hat) is str:
        U, V = adj_spectral_embedding(A_stripped, max_dim = int(round(log(n))))
        elbows = profile_likelihood(V, n_elbows = 2)
        X_hat = U[:, :elbows[1]] # @ np.diag(V[:elbows[-1]])**(1/2), adj_spectral_embedding already scales the vectors!
    
    likelihoods = gaussian_classification(X_hat, seeds, labels, return_likelihoods = True)

    final_labels = np.zeros(n)
    for i in range(len(unlabeled)):
        final_labels[unlabeled[i]] = np.argmax(likelihoods[i] * new_pis[i])
...
def get_p_multiple_KS(collection_of_weights1, collection_of_weights2, add_smoothing = 0):
